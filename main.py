...
txt = input("Введите номер для издателя (1, 2 , 3 или 4): ")
txt_num = int(txt)

q = session.query(Publisher)\
    .join(Book.publisher)\
    .filter(Publisher.id == txt_num)\
    .all()
...
# обновление объектов
session.query(Publisher).filter(Publisher.name == "O\u2019Reilly").update({"name": "O'Reilly"})
session.commit()
